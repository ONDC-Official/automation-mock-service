...
export async function setFlowAndTransactionId(
    ...
    try {
        ...
        if (!flowId) {
            logger.error(
                "Flow ID not found for " +
                    transactionService.createTransactionKey(
                        transactionId,
                        subscriberUrl,
                    ),
                getLoggerData(req),
        ...
        const sessionData =
            await new SessionCacheService().loadSessionThatExists(
...
export async function startNewFLow(
    ...
    try {
        ...
        const sessionData =
            await new SessionCacheService().loadSessionThatExists(sessionId);
        const flow = await fetchFlow(sessionData, flowId);
...
export async function ActUponFlow(req: ApiRequest, res: Response) {
    ...
    try {
        ...
        if (!flow || !txData || !subscriberUrl || !txId) {
            ...
            res.status(500).send(
                "<INTERNAL-ERROR> Flow or Transaction data not found",
            );
            return;
        }
        ...
        if (flowStatus.status === "SUSPENDED") {
            logger.info(
                "Flow is suspended, not proceeding",
                getLoggerData(req),
            );
            res.status(200).send({
                message: "Flow is suspended, not proceeding",
            });
            return;
        }
        if (flowStatus.status === "WORKING") {
            ...
            res.status(200).send({
                message:
                    "a flow response is already in progress, wait and try again!",
            });
            return;
        }
        console.log("flowStatus---------", flowStatus);
        ...
        if (!latestMeta) {
            ...
            res.status(200).send(
                "Mock response is not required flow is complete",
            );
            return;
        }
        ...
        if (
            ...
            if (
                ...
                latestMeta.actionType === "HTML_FORM_MULTI"
            ) {
                ...
                (sessionData as any)[firstKey] = req.body.inputs.submission_id;
                // Also save to form_data for on_status generators
                if (!(sessionData as any).form_data) {
                    (sessionData as any).form_data = {};
                }
                (sessionData as any).form_data[firstKey] = {
            ...
            const sessionData: any = await loadMockSessionData(
                txId,
                subscriberUrl,
            );
            ...
            for (let i = 0; i < 1; i++) {
                ...
                const playground =
                    req.apiSessionCache?.usecaseId === "PLAYGROUND-FLOW";
                await saveDataForConfig(
                    saveData,
                    mockResponse,
                    undefined,
                    playground,
