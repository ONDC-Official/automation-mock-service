...
const createQuoteFromItems = (items: any): any => {
  ...
  if (items[0]?.tags) {
    const farePolicy = items[0].tags.find(
      (tag: any) => tag.descriptor.code === "FARE_POLICY"
    );
    if (farePolicy) {
      const perKmCharge = farePolicy.list.find(
        (item: any) => item.descriptor.code === "PER_KM_CHARGE"
      );
      if (perKmCharge) {
        const distanceFare = {
          title: "DISTANCE_FARE",
          price: {
            ...
            value: (
              Number(items[0].price.value) -
              Number(
                farePolicy.list.find(
                  (item: any) => item.descriptor.code === "MIN_FARE"
                ).value
              )
            ).toFixed(2),
          },
