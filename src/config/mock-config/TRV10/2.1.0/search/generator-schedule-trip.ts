...
export async function searchMultipleStopsScheduleTripGenerator(
  existingPayload: any,
  sessionData: SessionData
) {
  const { city_code, end_gps, scheduled_time, start_gps } =
  ...
  if (end_gps) {
    const endStop = existingPayload.message.intent.fulfillment.stops.find(
      (s: any) => s.type === "END"
    );
    if (endStop) {
      endStop.location.gps = end_gps;
    }
  }
