...
export const issueStatusGenerator = async (
  ...
  const item = sessionData?.selected_item_id;
  const order = sessionData?.order_id;
  const fulfillment = sessionData?.fulfillment_id;
  const refs = [
    {
      ref_id: order ?? "Order_1",
      ref_type: "ORDER",
    },
    {
      ref_id: provider ?? "Provdier_1",
      ref_type: "PROVIDER",
    },
    {
      ref_id: fulfillment ?? "Fulfillment_1",
