...
import { createMockResponseRET15_125 } from "./APPLIANCES/1.2.5/generation-pipeline";
...
export async function createMockResponse(
  ...
  if (usecaseId === "APPLIANCES") {
    if (version === "1.2.5") {
      payload = await createMockResponseRET15_125(action_id, sessionData, input);
    } else {
