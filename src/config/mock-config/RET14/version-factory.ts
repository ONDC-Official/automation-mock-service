...
import { createMockResponseRET14_125 } from "./ELECTRONICS/1.2.5/generation-pipeline";
...
export async function createMockResponse(
  ...
  if (usecaseId === "ELECTRONICS") {
    if (version === "1.2.5") {
      payload = await createMockResponseRET14_125(action_id, sessionData, input);
    } else {
