...
import { createMockResponseRET16_125 } from "./HOME&KITCHEN/1.2.5/generation-pipeline";
...
export async function createMockResponse(
  ...
  if (usecaseId === "HOME&KITCHEN") {
    if (version === "1.2.5") {
      payload = await createMockResponseRET16_125(action_id, sessionData, input);
    } else {
