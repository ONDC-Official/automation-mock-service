...
import { SessionData as MockSessionData } from "./RET18/session-types";
import { createMockResponse } from "./RET18/version-factory";

export { MockSessionData };

const actionConfig = yaml.load(
  readFileSync(path.join(__dirname, "./RET18/factory.yaml"), "utf8")
) as any;

export const defaultSessionData = () => yaml.load(
  readFileSync(path.join(__dirname, "./RET18/session-data.yaml"), "utf8")
...
export function getSaveDataContent(version: string, action: string) {
  let actionFolderPath = path.resolve(
    __dirname,
    `./RET18/HEALTH&WELLNESS/${version}/${action}`
  );
