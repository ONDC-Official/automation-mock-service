...
import { SessionData as MockSessionData } from "./AGR10/session-types";
import { createMockResponse } from "./AGR10/version-factory";

export { MockSessionData };

const actionConfig = yaml.load(
	readFileSync(path.join(__dirname, "./AGR10/factory.yaml"), "utf8")
) as any;

export const defaultSessionData = () =>
	yaml.load(
		readFileSync(path.join(__dirname, "./AGR10/session-data.yaml"), "utf8")
...
export function getSaveDataContent(version: string, action: string) {
	let actionFolderPath = path.resolve(
		__dirname,
		`./AGR10/AGRI_INPUT/${version}/${action}`
	);
