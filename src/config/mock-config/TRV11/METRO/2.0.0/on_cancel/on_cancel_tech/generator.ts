		...
		const updatedTags = tags.map((tag: any) => {
			if (tag?.descriptor?.code === "SETTLEMENT_TERMS") {
				...
				const updatedList = list.map((item: any) => {
					if (
						item?.descriptor?.code === "SETTLEMENT_AMOUNT"
					) {
						return {
							...item,
							value: "0",
...
function applyCancellation(quote: Quote, cancellationCharges: number): Quote {
  ...
  export async function onCancelTechCancelGenerator(existingPayload: any,sessionData: any){
    if (sessionData.updated_payments.length > 0) {
      existingPayload.message.order.payments = sessionData.updated_payments;
      existingPayload.message.order.payments = markSettlementAmountZero(existingPayload.message.order.payments);
      }
