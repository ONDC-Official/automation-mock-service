...
export const issueStatusGenerator_100 = async (
  ...
  let fulfillment = "F1";
  if (sessionData.fulfillment_ids) {
    fulfillment = sessionData?.fulfillment_ids[0];
  }
  ...
  switch (sessionData.igm_action) {
    case "issue_open":
      ...
      existingPayload.message.issue.complainant_info = {
        ...
        contact: {
          phone: sessionData?.billing?.phone ?? "9879879870",
      ...
      existingPayload.message.issue.order_details = {
        ...
        provider_id: provider ?? "Provider1",
      };
        ...
        complainant_actions: [
          {
            ...
            updated_by: {
              ...
              contact: {
                phone: sessionData?.billing?.phone ?? "9450394039",
                email: sessionData?.billing?.email ?? "buyerapp@interface.com",
              },
              person: {
                name: sessionData?.billing?.name ?? "John Doe",
              },
            },
          },
        ],
        ...
        complainant_actions: [
          ...sessionData?.issue_actions?.complainant_actions,
          {
            ...
            updated_by: {
              ...
              contact: {
                phone: sessionData?.billing?.phone ?? "9450394039",
                email: sessionData?.billing?.email ?? "interfacingapp@interface.com",
              },
              person: {
                name: sessionData?.billing?.name ?? "John Doe",
              },
