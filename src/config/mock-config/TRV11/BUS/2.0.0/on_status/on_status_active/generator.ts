...
export async function onStatusActiveGenerator(existingPayload: any,sessionData: SessionData){
    ...
    existingPayload.message.order.fulfillments = sessionData.on_confirm_fulfillment?.flat();
    }
...
export async function onStatusCompletedGenerator(existingPayload: any,sessionData: SessionData){
    if (sessionData.updated_payments.length > 0) {
        existingPayload.message.order.payments = sessionData.updated_payments;
      }
