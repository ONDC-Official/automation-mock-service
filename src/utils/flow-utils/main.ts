...
export function computeSubscriber(context: BecknContext) {
	...
	if (action.startsWith("on")) {
		if (!context.bpp_uri) {
			...
			logInfo({
				message: "Exiting computeSubscriber Function. BPP URI is not present in the context",
				meta: { action },
				transaction_id: context.transaction_id,
			});
		...
		// );
		logInfo({
			message: "Exiting computeSubscriber Function. Computed subscriber for action " + action + " is " + context.bpp_uri,
	...
	// );
	logInfo({
		message: "Exiting computeSubscriber Function. Computed subscriber for action " + action + " is " + context.bap_uri,
